...
// --- AI ENGINE CONTRACT TYPES ---

export interface AIContext {
  role?: UserRole;
  userName?: string;
  historyContext?: string; // Semantic summary from previous interactions
}
...
export interface DensityIssue {
  stepId: string;
  type: 'over_expanded' | 'under_supported';
  severity: 'critical' | 'minor';
  feedback?: string; 
}

export interface ReasoningDensity {
  issues: DensityIssue[];
  overallDensity: 'sparse' | 'balanced' | 'dense' | 'irregular';
}

export interface VisualAid {
  type: 'diagram_description' | 'graph_description' | 'illustration_prompt';
  title: string;
  content: string;
}
...
export interface AnalysisResult {
  ...
  interpretationStability?: InterpretationStability; 
  globalReasoning?: GlobalReasoningStructure; 
  assumptionIntegrity?: AssumptionIntegrity; 
...
// Legacy mapping for Orchestrator/UI compatibility (will be deprecated)
